...
* PHP
* Apache (Now replaced with Nginx)
* MySQL
* phpMyAdmin
* Redis
* ElasticSearch
...
I have set 3 webservers

* webserver-tm (tm.co.pk)
* webserver-bm (bm.co.pk)
* webserver-waf (waf.co.pk)

So to connect to related webserver, specify the webserver name while connecting
e-g
for webserver-bm performing ssh as root
#### `root` User
```
docker-compose exec webserver-bm bash
```

to perform ssh as www-data user following command can be used for webserver-bm
#### `www-data` User
```shell
sudo docker exec -it --user www-data webserver-bm /bin/bash
```
